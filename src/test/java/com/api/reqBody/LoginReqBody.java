...
//	}
	...
	public static void postRequestLogin_WithInvalid_EndPoint() throws IOException {
		response = request
				.body(loginjsonReaderObj.Invalid_EndPoint(path.getString("loginJsonFilePath"), "post_invalidEndPoint"))
				.post(endpoint.getString("Invalid_EndPoint"));

	}

	public static void post_invalid_username() throws IOException {
		response = request.body(loginjsonReaderObj.Invalid_UserName_json_Reader(path.getString("loginJsonFilePath"),
				"post_invalidLoginUserName")).post(endpoint.getString("login"));
	}

	public static void post_invalid_Password() throws IOException {
		response = request.body(loginjsonReaderObj.Invalid_Password_json_Reader(path.getString("loginJsonFilePath"),
				"post_invalidLoginPassword")).post(endpoint.getString("login"));
	}

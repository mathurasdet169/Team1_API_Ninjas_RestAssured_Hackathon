...
public class CommonUtils {
	...
	public static List<BatchPayload> lstobjBData = new ArrayList<BatchPayload>();
	static BatchReqBody batchReqBodyObj = new BatchReqBody();

	public static void setBaseRequest_withoutBearer() {

		request = RestAssured.given().baseUri(baseURI).header("Content-Type", "application/json")
	...
	public enum ProgramScenarioType {
		...
		GET_PROGIDNOAUTH_METHOD("getProgId_NoAuth"), GETALL_ADMIN_INVALID_METHOD("getAllAdmin_InvalidMethod"),
		GETALL_ADMIN_NOAUTH_METHOD("getAllAdmin_NoAuth"), PUT_PROGID_INVALID_METHOD("putProgId_InvalidMethod"),
		PUT_PROGID_NOAUTH_METHOD("putProgId_NoAuth"), PUT_PROGNAME_NOAUTH_METHOD("putProgName_NoAuth"),
		POST_ADDPROGRAM_METHOD("post_addProgram"), DELETE_PROGNAME_NOAUTH_METHOD("deleteProgNam_NoAuth"),
		DELETE_PROGID_NOAUTH_METHOD("deleteProgId_NoAuth"), DELETE_PROGNAME("deleteProgName_ValidProgName"),
		DELETE_PROGID("deleteProgId_ValidProgId"), DELETE("delete");
	...
	public static void dataCleanUp() throws IOException {
		LoggerLoad.info("Delete All Program/Batch method called: ");
		...
		if (lstobjPData != null && lstobjPData.size() > 0) {
			...
			RequestSpecification request = RestAssured.given().header("Content-Type", "application/json")
					.header("Authorization", "Bearer " + bearerToken).log().all();
			...
			if (lstobjBData != null && lstobjBData.size() > 0) {
				LoggerLoad.info("List of batch IDs: " + lstobjBData.size());

				Iterator<BatchPayload> batchIterator = lstobjBData.iterator();
				while (batchIterator.hasNext()) {

					BatchPayload objBdata = batchIterator.next();

					LoggerLoad.info("BatchId : " + objBdata.getBatchId());
				...
				LoggerLoad.info("List of Program/batch IDs while exiting: " + "batch" + lstobjBData.size() + "program"
						+ lstobjPData.size());

				LoggerLoad.info("Delete All Program/batch ID method Exited: ");
